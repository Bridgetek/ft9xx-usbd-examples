...
/** @brief Camera format.
 * @details One of CAMERA_FORMAT_UNCOMPRESSED/MJPEG.
 */
...
uint16_t camera_init(void)
{
	...
	else
	{
		...
		if (type != 0)
		{
			...
			CAMERA_start_fn = ov965x_start;
			CAMERA_stop_fn = ov965x_stop;
			CAMERA_set_fn = ov965x_set;
			CAMERA_supports_fn = ov965x_supports;
		}
	}
...
static uint8_t frame_idx_mjpeg = 0;

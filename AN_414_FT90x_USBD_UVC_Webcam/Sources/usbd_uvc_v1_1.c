...
/** @brief Format index for MJPEG streams.
 */
uint8_t uvc_format_index_mjpeg = 0;
...
int8_t class_vs_check_probecommit(USB_UVC_VideoProbeAndCommitControls *probecommit)
{
	...
	if (usb_speed == USBD_SPEED_HIGH)
	{
		...
		else if (probecommit->bFormatIndex == uvc_format_index_mjpeg)
		{
...
int8_t class_vs_set_commit(USB_UVC_VideoProbeAndCommitControls *commit)
{
			...
			(commit->bPreferedVersion == USB_VIDEO_CLASS_VERSION_MINOR))
	{
		...
		else if (commit->bFormatIndex == uvc_format_index_mjpeg)
		{
			format = CAMERA_FORMAT_MJPEG;
		}
		else
...
int8_t usb_uvc_is_mjpeg()
{
	if ((uvc_commit.bFormatIndex > FORMAT_INDEX_TYPE_NONE)
			&& (uvc_commit.bFormatIndex < FORMAT_INDEX_MAX))
	{
		return (uvc_commit.bFormatIndex == uvc_format_index_mjpeg);
	}
	return 0;
}
...
void usb_uvc_build_configuration(uint16_t module)
{
	...
	uint8_t countFrameMJPEG = camera_mode_get_frame_count(CAMERA_FORMAT_MJPEG);
	uint8_t countFormats = MIN(countFrameUncompressed, 1) +
			MIN(countFrameMJPEG, 1);
	uint16_t countFrameRatesUncompressed = 0;
	uint16_t countFrameRatesMJPEG = 0;
	...
	for (i = 0; i < countFrameMJPEG; i++)
	{
		countFrameRatesMJPEG += camera_mode_get_frame_rate_count(CAMERA_FORMAT_MJPEG, i);
	}
	...
	if (countFrameMJPEG)
	{
		len_hs += sizeof(USB_UVC_VS_MJPEGVideoFormatDescriptor) +
				(sizeof(USB_UVC_VS_MJPEGVideoFrameDescriptorDiscrete(0)) * countFrameMJPEG) +
				sizeof(USB_UVC_ColorMatchingDescriptor);
		len_hs += (sizeof(unsigned long) * countFrameRatesMJPEG);
	}
	...
	config_descriptor_buffer_hs = malloc(len_hs);
	config_descriptor_buffer_fs = malloc(len_fs);
...

	{
		...
		if (countFrameMJPEG)
		{
			uvc_format_index_mjpeg = countFormats;

			USB_UVC_VS_MJPEGVideoFormatDescriptor c = {
					...
					countFormats, /* format.bFormatIndex */
					countFrameMJPEG, /* format.bNumFrameDescriptors */
					0, /* format.bmFlags */
					1, /* format.bDefaultFrameIndex */
					FRAME_RATIO_X, /* format.bAspectRatioX */
					FRAME_RATIO_Y, /* format.bAspectRatioY */
			...
			do
			{
				frame_index = camera_mode_get_frame(CAMERA_FORMAT_MJPEG, countFrameMJPEG,
						&width, &height);
				if (frame_index == 0)
				{
					break;
				...
				countFrameRatesMJPEG = camera_mode_get_frame_rate_count(CAMERA_FORMAT_MJPEG,
						countFrameMJPEG);
				/* Get first frame rate for default. */
				frame_rate = camera_mode_get_frame_rate(CAMERA_FORMAT_MJPEG, countFrameMJPEG,
						0);
				{
					USB_UVC_VS_MJPEGVideoFrameDescriptorDiscrete(0) c = {
							sizeof(USB_UVC_VS_MJPEGVideoFrameDescriptorDiscrete(countFrameRatesMJPEG)), /* frame.bLength */
							USB_UVC_DESCRIPTOR_TYPE_CS_INTERFACE, /* frame.bDescriptorType */
							...
							(width * height * FORMAT_UC_BBP), /* frame.dwMaxVideoFrameBufferSize */
							10000000 / frame_rate, /* frame.dwDefaultFrameInterval */
							countFrameRatesMJPEG, /* frame.bFrameIntervalType */
					};

					for (i = 0; i < countFrameRatesMJPEG; i++)
					{
						frame_rate = camera_mode_get_frame_rate(CAMERA_FORMAT_MJPEG,
								countFrameMJPEG, i);
						c.dwFrameInterval[i] = 10000000 / frame_rate; /* frame.dwFrameInterval */
					}

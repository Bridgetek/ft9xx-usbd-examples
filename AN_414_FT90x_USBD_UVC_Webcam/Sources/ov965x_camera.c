...
uint16_t ov965x_init(void)
{
	...
	do {
		...
		err = i2cm_read(OV965x_I2C_ADDR, REG_PID, &pidh, 1);
		if (err) break;
		err = i2cm_read(OV965x_I2C_ADDR, REG_VER, &pidl, 1);
		if (err) break;

		CAMERA_DEBUG_PRINTF("OV965x version: %x\r\n", ((pidh << 8) | pidl));
...
int8_t ov965x_supports(uint16_t width, uint16_t height, int8_t frame_rate, int8_t format)
{
	int8_t ret = -1;
	...
	if (format == CAMERA_FORMAT_UNCOMPRESSED)
	{
		...
		else if ((width == CAMERA_FRAME_WIDTH_SXGA)
				&& (height == CAMERA_FRAME_HEIGHT_SXGA))
		{
			if ((frame_rate == 15) || (frame_rate == CAMERA_FRAME_RATE_ANY))
			{
				ret = 0;
...
int8_t ov965x_set(uint16_t width, uint16_t height, int8_t format,
	...
	if (format == CAMERA_FORMAT_UNCOMPRESSED)
	{
				...
				&& (height == CAMERA_FRAME_HEIGHT_QVGA))
		{
...

			{
				...
				// Sample size is 1 complete line - 640 bytes.
				*sample_size = CAMERA_FRAME_WIDTH_QVGA * OV965x_BBP;
				*frame_size = CAMERA_FRAME_WIDTH_QVGA * CAMERA_FRAME_HEIGHT_QVGA * OV965x_BBP;
				*frame_rate = 15;
				ret = 0;
			}
		}
				...
				&& (height == CAMERA_FRAME_HEIGHT_VGA))
		{
			...
			if ((*frame_rate == 15) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
			{
				...
				*sample_size = ((CAMERA_FRAME_WIDTH_VGA * OV965x_BBP));
				*frame_size = CAMERA_FRAME_WIDTH_VGA * CAMERA_FRAME_HEIGHT_VGA * OV965x_BBP;
				*frame_rate = 15;
				ret = 0;
			}
		}
				...
				&& (height == CAMERA_FRAME_HEIGHT_SVGA))
		{
			...
			if ((*frame_rate == 15) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
			{
				...
				// Sample size is 1/2 a line - 800 bytes.
				*sample_size = ((CAMERA_FRAME_WIDTH_SVGA * OV965x_BBP) / 2);
				*frame_size = CAMERA_FRAME_WIDTH_SVGA * CAMERA_FRAME_HEIGHT_SVGA * OV965x_BBP;
				*frame_rate = 15;
				ret = 0;
			}
		}
				...
				&& (height == CAMERA_FRAME_HEIGHT_XGA))
		{
			...
			if ((*frame_rate == 15) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
			{
				...
				// Sample size is 1/2 a line - 1024 bytes.
				*sample_size = ((CAMERA_FRAME_WIDTH_XGA * OV965x_BBP) / 2);
				*frame_size = CAMERA_FRAME_WIDTH_XGA * CAMERA_FRAME_HEIGHT_XGA * OV965x_BBP;
				*frame_rate = 15;
				ret = 0;
			}
		}
		// SXGA supports uncompressed only at 15 fps.
				...
				&& (height == CAMERA_FRAME_HEIGHT_SXGA))
		{
			CAMERA_DEBUG_PRINTF(" SXGA");
			if ((*frame_rate == 15) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
			{
				CAMERA_DEBUG_PRINTF(" 15fps");
				ov965x_send(ov965x_sxga_regs, sizeof(ov965x_sxga_regs));
				// Sample size is 1/4 a line - 640 bytes.
				*sample_size = ((CAMERA_FRAME_WIDTH_SXGA * OV965x_BBP) / 4);
				*frame_size = CAMERA_FRAME_WIDTH_SXGA * CAMERA_FRAME_HEIGHT_SXGA * OV965x_BBP;
				*frame_rate = 15;
				ret = 0;
			}
		}

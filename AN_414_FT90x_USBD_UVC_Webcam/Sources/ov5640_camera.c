...
{
	register uint8_t status = I2CM->I2CM_CNTL_STATUS;

	while ((status = I2CM->I2CM_CNTL_STATUS) & I2C_STATUS_BUSY)
        ; /* Wait for the bus to be idle */

	return (status & I2C_STATUS_ERROR);
}
...
static int8_t ov5640_writesccb(__flash__ const struct ov5640_reg *ptb, int32_t len)
{
	...
	for (i = 0; i < len; i++) {
		...
		else
		{
			...
			if (regv != ptb[i].data)
			{
				CAMERA_DEBUG_PRINTF("%d 0x%04x 0x%02x (0x%02x)\n", ret, ptb[i].addr, regv, ptb[i].data);
			}
#endif
		}
	}
...
static __flash__ const struct ov5640_reg configscript_reset[] = {
		/* System Control */
		{0x3103, 0x11},
		{0x3008, 0x82},
...
static __flash__ const struct ov5640_reg common_config_1[]=
{
		{0x3103, 0x03},
		{0x3017, 0xff},
		{0x3018, 0xff},
...
static __flash__ const struct ov5640_reg set_qvga_30fps[] = {
		{0x3800,MSB(0)},
		{0x3801,LSB(0)},
		{0x3802,MSB(4)},
		{0x3803,LSB(4)},
		...
		{0x3813,LSB(2)},
		// For MJPEG output.
		{0x4602,MSB(OV5640_PREFERRED_LINE_LENGTH)},
		{0x4603,LSB(OV5640_PREFERRED_LINE_LENGTH)},
		{0x4604,MSB(OV5640_LINE_COUNT_MJPEG_QVGA)},
		{0x4605,LSB(OV5640_LINE_COUNT_MJPEG_QVGA)},
};

static __flash__ const struct ov5640_reg set_vga_30fps[] = {
		{0x3800,MSB(0)},
		{0x3801,LSB(0)},
		{0x3802,MSB(4)},
		{0x3803,LSB(4)},
...
static __flash__ const struct ov5640_reg set_svga_30fps[] = {
		{0x3800,MSB(0)},
		{0x3801,LSB(0)},
		{0x3802,MSB(4)},
		{0x3803,LSB(4)},
...
static __flash__ const struct ov5640_reg set_xga_30fps[] = {
		{0x3800,MSB(0)},
		{0x3801,LSB(0)},
		{0x3802,MSB(4)},
		{0x3803,LSB(4)},
...
static __flash__ const struct ov5640_reg set_jpeg[]= {
		{0x3034, 0x1a},
		{0x3035, 0x11},
		{0x3036, 0x46},
		{0x3631, 0x0e},
...
static __flash__ const struct ov5640_reg set_yuv[]= {
		{0x3034, 0x1a},
		{0x3035, 0x31},
		{0x3036, 0x69},
		//{0x3631, 0x0e},
...
static __flash__ const struct ov5640_reg common_config_2[]=
{
		{0x3618, 0x00},
		{0x3612, 0x29},
		{0x3708, 0x66},
...
static __flash__ const struct ov5640_reg ov5640_stream[] = {
		{0x300e, 0x59},
		{0x3008, 0x02},
};

static __flash__ const struct ov5640_reg ov5640_power_down[] = {
		{0x3008, 0x42},
		{0xFFFF, 100},
};
...
uint16_t ov5640_init(void)
{
	...
	do
	{
		...
		CAMERA_DEBUG_PRINTF("OV5640 version: %x\r\n", ((pidh << 8) | pidl));
...
int8_t ov5640_supports(uint16_t width, uint16_t height, int8_t frame_rate, int8_t format)
{
	...
	// QVGA supports uncompressed and MJPEG at 30 fps.
	if ((width == CAMERA_FRAME_WIDTH_QVGA)
			&& (height == CAMERA_FRAME_HEIGHT_QVGA))
	{
		if ((frame_rate == 30) || (frame_rate == CAMERA_FRAME_RATE_ANY))
		{
	...
	// VGA supports uncompressed and MJPEG at 30 fps.
	else if ((width == CAMERA_FRAME_WIDTH_VGA)
	...
	// SVGA supports MJPEG only at 30 fps.
	else if ((width == CAMERA_FRAME_WIDTH_SVGA)
			&& (height == CAMERA_FRAME_HEIGHT_SVGA))
	{
		if ((frame_rate == 30) || (frame_rate == CAMERA_FRAME_RATE_ANY))
		{
	...
	// SVGA supports MJPEG only at 30 fps.
	else if ((width == CAMERA_FRAME_WIDTH_XGA)
			&& (height == CAMERA_FRAME_HEIGHT_XGA))
	{
		if ((frame_rate == 30) || (frame_rate == CAMERA_FRAME_RATE_ANY))
		{
			if (format == CAMERA_FORMAT_MJPEG)
		...
		int8_t *frame_rate, uint16_t *sample_size, uint32_t *frame_size)
{
	...
	if ((width == CAMERA_FRAME_WIDTH_QVGA)
			&& (height == CAMERA_FRAME_HEIGHT_QVGA))
	{
		CAMERA_DEBUG_PRINTF(" QVGA");
		if ((*frame_rate == 30) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
		{
			CAMERA_DEBUG_PRINTF(" 30fps");
			OV5640_WRITESCCBREG(set_qvga_30fps);

			if (format == CAMERA_FORMAT_UNCOMPRESSED)
			{
				CAMERA_DEBUG_PRINTF(" uncompressed");
				OV5640_WRITESCCBREG(set_yuv);
				*sample_size = CAMERA_FRAME_WIDTH_QVGA * OV5640_BBP;
				*frame_size = CAMERA_FRAME_WIDTH_QVGA * CAMERA_FRAME_HEIGHT_QVGA * OV5640_BBP;
				*frame_rate = 30;
				ret = 0;
			}
			else if (format == CAMERA_FORMAT_MJPEG)
			{
				CAMERA_DEBUG_PRINTF(" MJPEG");
				OV5640_WRITESCCBREG(set_jpeg);
				*sample_size = OV5640_PREFERRED_LINE_LENGTH;
				*frame_size = OV5640_FRAME_SIZE_MJPEG_QVGA;
				*frame_rate = 30;
				ret = 0;
			}
		}
			...
			&& (height == CAMERA_FRAME_HEIGHT_VGA))
	{
		...
		{
			CAMERA_DEBUG_PRINTF(" 30fps");
			OV5640_WRITESCCBREG(set_vga_30fps);

			if (format == CAMERA_FORMAT_UNCOMPRESSED)
			{
				CAMERA_DEBUG_PRINTF(" uncompressed");
				OV5640_WRITESCCBREG(set_yuv);
				*sample_size = CAMERA_FRAME_WIDTH_VGA * OV5640_BBP;
				*frame_size = CAMERA_FRAME_WIDTH_VGA * CAMERA_FRAME_HEIGHT_VGA * OV5640_BBP;
				*frame_rate = 30;
				ret = 0;
			}
			else if (format == CAMERA_FORMAT_MJPEG)
			{
				...
				OV5640_WRITESCCBREG(set_jpeg);
				*sample_size = OV5640_PREFERRED_LINE_LENGTH;
				*frame_size = OV5640_FRAME_SIZE_MJPEG_VGA;
				*frame_rate = 30;
				ret = 0;
			}
		}
	...
	// SVGA supports MJPEG only at 30 fps.
			...
			&& (height == CAMERA_FRAME_HEIGHT_SVGA))
	{
		CAMERA_DEBUG_PRINTF(" SVGA");
		if ((*frame_rate == 30) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
		{
			CAMERA_DEBUG_PRINTF(" 30fps");
			OV5640_WRITESCCBREG(set_svga_30fps);
			...
			if (format == CAMERA_FORMAT_MJPEG)
			{
				...
				OV5640_WRITESCCBREG(set_jpeg);
				*sample_size = OV5640_PREFERRED_LINE_LENGTH;
				*frame_size = OV5640_FRAME_SIZE_MJPEG_SVGA;
				*frame_rate = 30;
				ret = 0;
			}
		}
	...
	// SVGA supports MJPEG only at 30 fps.
			...
			&& (height == CAMERA_FRAME_HEIGHT_XGA))
	{
		CAMERA_DEBUG_PRINTF(" XGA");
		if ((*frame_rate == 30) || (*frame_rate == CAMERA_FRAME_RATE_ANY))
		{
			CAMERA_DEBUG_PRINTF(" 30fps");
			OV5640_WRITESCCBREG(set_xga_30fps);
			...
			if (format == CAMERA_FORMAT_MJPEG)
			{
				...
				OV5640_WRITESCCBREG(set_jpeg);
				*sample_size = OV5640_PREFERRED_LINE_LENGTH;
				*frame_size = OV5640_FRAME_SIZE_MJPEG_XGA;
				*frame_rate = 30;
				ret = 0;
			}
		}

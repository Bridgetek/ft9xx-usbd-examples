...
static const uint8_t __flash__ img_bridgetek_logo[] __attribute__((aligned(4))) =
{
...
uint32_t eve_load_images(uint32_t start_addr)
{
	...
	while (flag != 2)
	{
		memcpy_pm2dat(buf, (const __flash__ void *)eve_addr, sizeof(buf));
	...
	EVE_BITMAP_LAYOUT(RGB565, img_width * 2, img_height);
